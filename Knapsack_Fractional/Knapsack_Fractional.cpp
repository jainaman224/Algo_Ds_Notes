...
double fractionalKnapsackProblem(vector<pair<double,pair<double,double> > > arr, int n, int W)
{
    ...
    for (int i = (n - 1); i >= 0; i--)
    {
        ...
        if ((weight + arr[i].second.second) <= W) 
        {
...
int main()
{
    ...
    double value = fractionalKnapsackProblem(arr, n, W);
