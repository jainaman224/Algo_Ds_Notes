...
int partition_index(int array[],int low,int high){
    ...
    for(int j = low; j < high; j++){
        if(array[j] <= pivot){                 // If current element is smaller than or  
            i++;                               // equal to pivot then increment smaller index and swap
...
// This function generates a random_index in the range [low,high] and swaps array[high] with array[random_index]
int random_partition(int array[],int low,int high){
    srand(time(NULL));    // To create a random number every time the program is executed
    int random_index = low + rand()%(high - low);   //random_index in the range [low,high]
