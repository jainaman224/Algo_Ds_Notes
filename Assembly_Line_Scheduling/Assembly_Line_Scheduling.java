/*
    ...
    Java Program for Assembly Line Scheduling (Dynamic Programming). 
...
class Assembly_Line_Scheduling 
{
    ...
    public static void main (String[] args)
    {
        ...
        */
        ...
        System.out.print("Enter the no. of nodes on Assembly line : ");
        ...
        {
            System.out.print("For Line" + (lineN + 1) + ": ");
            inTime[lineN] = s.nextInt();
        }
        ...
        for (int lineN = 0; lineN < assemblyLines; lineN++)
        {
            ...
            {
                System.out.print("Node" + nThNode + ": ");
                line[lineN][nThNode] = s.nextInt();
            }
        }
        ...
        for (int lineN = 0; lineN < assemblyLines; lineN++)
        {
            for (int nThNode = 0; nThNode < nodes; nThNode++)
            {
                ...
                else
                {
                    if (lineN == 0)
                    {
                        System.out.print("Node" + (lineN + 1) + "(" + (line[lineN][nThNode - 1]) + ") to Node" + (lineN + 2) + "(" + (line[lineN + 1][nThNode]) + ") : ");
                        tTime[lineN][nThNode] = s.nextInt();
                    }   
                    else
                    {
                        System.out.print("Node" + (lineN + 1) + "(" + (line[lineN][nThNode - 1]) + ") to Node" + (lineN) + "(" + (line[lineN - 1][nThNode]) + ") : ");
                        tTime[lineN][nThNode] = s.nextInt();
                    }
                }
            }
        ...
        for (int node = 0; node < (nodes + 2); node++)
        {
            if (node == 0)
            {
                System.out.print("Path : In" + (path[node]+1) + "(" + inTime[path[node]] + ") --> ");	
            }
            else if (node == nodes + 1)
            {
                System.out.println("Out" + (path[node] + 1) + "(" + outTime[path[node]] + ")");
            }
            else
            {
                System.out.print("Node" + (path[node] + 1) + "(" + line[path[node]][node - 1] + ") --> ");
            }
        }
    }
}
