...
inline static const std::unordered_map<Apology, std::string_view> Apologies = {
...
inline auto show(const Apology &apology) -> void {
    switch (apology) {
    case Apology::empty: {
...
template <class Ty> class bst_ {
  ...
  public:
    ...
    constexpr auto insert(Ty &&val) -> void {
        ...
        auto insert_hidden = [this](Ty &&val, sh_ptr &root, const node *par,
            ...
            if (!root) {
                ...
                lambda(std::move(val), root->m_right, root.get(),
                       std::move(lambda));
            } else if (val < root->m_data) {
    ...
    constexpr auto remove(Ty &&val) -> void {
        ...
        //
        --m_size;
        auto remove_hidden = [this](Ty &&val, sh_ptr &root,
                                    auto &&lambda) -> void {
            if (!root) {
                return;
            } else if (val > root->m_data) {
...
int main() {
    ...
    std::cout << nums.contains(77) << '\n';
    std::cout << nums.depth(77) << '\n';
    std::cout << *nums.get_parent(88) << '\n';
    /// and more
}
